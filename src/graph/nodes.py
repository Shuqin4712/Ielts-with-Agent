...
from .state import CRITERIA, DimensionScore, GradeState
...
def score_dimensions(state: GradeState) -> dict:
    """顺序遍历四维打分（阶段 1 不并行）。档位与锚点由 run_cfg / state 决定。"""
    cfg = state.get("run_cfg") or {}
    ...
    scores = {crit: _score_criterion(crit, state, llm, anchors) for crit in CRITERIA}
...
def route_reflection(state: GradeState) -> str:
