...
检索层另有分环节评测（v1.3）：27 条零泄漏查询 × 三档检索策略，实证纯向量召回同话题命中仅 ~34%、生产路径的 metadata 过滤是必要设计（Recall@5 / MRR@10）。**v1.4** 进一步让向量真正进入打分路径——锚点从「band 均匀采样」换成「池内向量排序 + 跨 band 铺开」，单变量消融（各 2 次）实证 QWK 两次 {0.611, 0.616} 不与基线 {0.579, 0.604} 重叠、其余指标无退化，已设为生产默认。

完整评测方法、消融取舍、并行化质量回归、检索层评测、已知局限 → **[docs/EVALUATION.md](docs/EVALUATION.md)**
...
三个可深讲的技术点：**结构感知 RAG**（rubric 按 `(criterion, band)` 切 + metadata 过滤检索；范文锚点先 metadata 过滤、再池内向量排序选跨 band 校准梯子）、**跨会话记忆**（episodic + semantic 学生画像）、**量化评测**（gold-only、锚定消融、选锚消融、QWK）。**打分路径物理隔离**：个性化/记忆在外层图、判分在内层图，学生画像物理上进不了判分逻辑，反馈个性化绝不改分——评测走的正是这条内层管道。
